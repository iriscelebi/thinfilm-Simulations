...
for h=1:length(bioarray)
    ...
    R(h)=sum((Color(:,1).*Gamma),1)+0.1.*sum((Color(:,2).*Gamma),1);               
end
R0 = (R(1).*ones(1,length(R)));
ERR =(abs(R).^2-abs(R0).^2)./abs(R0).^2;
plot(bioarray,(ERR),'linewidth',1.5)
end
